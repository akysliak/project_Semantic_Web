    ...
    <body>
        ...
        <form>
            ...
            <select id="scientificName" name="scientificName" onchange= "submit()">
                <c:forEach items="${scientificNames}" var = "scName" >
                    <option value="${scName}" ${scientificName == scName ? 'selected' : ''}><%= MSWQuery.getLabel((String)pageContext.findAttribute("scName")) %></option>
                </c:forEach>
            </select>
        </form>
        <form>
        ...
        <c:if test="${show_results_scName}">
            <form>
                ...
                <h2><%= MSWQuery.getLabel((res_scName.get("scientific_name").getFirst())) %></h2>
