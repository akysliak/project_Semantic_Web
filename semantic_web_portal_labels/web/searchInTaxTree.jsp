        ...
        for(int i = 0; i < levelsNames.length; ++i) {
            ...
            if(el_selected!=null) {
                if(el_selected.equals("none")) {
                    ...
                    tTree.setSelectedLevel(el_selected); // deprecate
                   ...
                   <td>
                       <select id="${level}_selected" name="${level}_selected" style="width: 20em;" onchange='submit()'>
                            ...
                            <c:forEach items="${sessionScope[element_list]}" var = "el" >
                                <option value="${el}" ${el == sessionScope[selected_element_name] ? 'selected' : ''}><%= MSWQuery.getLabel((String)pageContext.findAttribute("el")) %></option>
                            </c:forEach>
                        </select>
                   </td>
                </tr>
